...
download_table_from_p3 <- function(csv_table, dest_folder = "data-raw") {
  url_table <- glue::glue("http://p3.snf.ch/P3Export/{csv_table}") %>%
...
specs <-
  ...
  map(read_csv)
names(specs) <- files
openxlsx::write.xlsx(specs, file = "data/specs.xlsx", overwrite = TRUE)
```
...
write_ods(
  projects,
  here("data-raw", "research-earth.ods"),
  sheet = "projects-p3")
```
...
projects_tags %>%
  write_ods(here("data-raw", "research-earth.ods"),
            sheet = "project-tag-helper",
            append = TRUE)
...
projects_disciplines_id %>%
  write_ods(here("data-raw", "research-earth.ods"),
            sheet = "project-disciplines_id-helper",
            append = TRUE)
```


- Add email addresses from r4d website.

```{r email-addresses, eval=FALSE}
# list of project urls
# HACK (bypassing data in javascript script !CDATA)
html_page <- xml2::read_html("http://www.r4d.ch/r4d-programme/all-projects")
r4d_urls <- html_page %>%
  # HACK bypass data in javascript script !CDATA with node:body > text
  rvest::html_nodes("body") %>%
  rvest::html_text() %>%
  str_extract_all("http[:][/][/]www[.]r4d[.]ch[/]modules[/][-/a-z]+") %>%
  unlist()

# get email for each project
# https://stackoverflow.com/questions/19341554
# proj_url <- "http://www.r4d.ch/modules/food-security/gender-and-the-right-to-food"
get_email <- function(proj_url) {
  message(proj_url)
  html_page <- xml2::read_html(proj_url)
  body_txt <- html_page %>%
    # HACK bypass data in javascript script body > text
    rvest::html_nodes("body") %>%
    rvest::html_text()
  email <-
    body_txt %>%
  ...
  message(email)
  p3_links <- html_page %>%
    rvest::html_nodes("ul a") %>%
    rvest::html_attr("href") %>%
    str_subset("http[:][/][/]p3[.]snf[.]ch") %>%
    unique()
  message(p3_links)
  df_links <- tibble(p3_url = p3_links) %>%
    mutate(contact = email)
  return(df_links)
}

r4d_emails <- map_df(r4d_urls, get_email)

email_addr <- r4d_emails %>%
  mutate(
    p3_url = str_remove(p3_url, "#"),
    contact = str_to_lower(contact),
    project_number = str_extract(p3_url, "[0-9]+$")) %>%
  select(-p3_url) %>%
  relocate(project_number)

write_csv(email_addr, here::here("data-raw", "email_addresses.csv"))
```
...
scrape_p3_project <- function(project_number) {
  message(glue::glue("* Scrapping {project_number}"))
  Sys.sleep(.25)

  p3_url <- glue::glue("http://p3.snf.ch/project-{project_number}")
...
projects_organizations_id %>%
  write_ods(here("data-raw", "research-earth.ods"),
            sheet = "project-organizations_id-helper",
            append = TRUE)
```

```{r institute-details, eval=FALSE}
scrape_p3_institute <- function(institute_number) {
  message(glue::glue("* Scrapping /institute-{institute_number}"))
  Sys.sleep(.25)
  p3_url <- glue::glue("http://p3.snf.ch/institute-{institute_number}")
  html_page <- xml2::read_html(p3_url)
  institute_details <-
    html_page %>%
...
p3_institute_details %>%
  select(institute_id, institute_p3_name) %>%
  distinct() %>%
  write_ods(here("data-raw", "research-earth.ods"),
            sheet = "institute-details-p3",
            append = TRUE)
...
people %>%
  relocate(person_id_snsf) %>%
  distinct(person_id_snsf, project_number, role, .keep_all = TRUE) %>%
  write_ods(here("data-raw", "research-earth.ods"),
            sheet = "people-p3",
            append = TRUE)
...
# tests :)
people %>%
  anti_join(p3_project_details, by = c("person_id_snsf" = "person_id_p3"))
p3_project_details %>%
  anti_join(people, by = c("person_id_p3" = "person_id_snsf"))
```
...
projects_re <-
  ...
  mutate(
    project__id = as.character(project__id),
    email = "",
    email = ifelse(project_description == "SPIRIT", "spirit@snf.ch", email),
    email = ifelse(project_description == "r4d", "r4d@snf.ch", email),
    projects_funder_id = "1",
    start_date = str_replace(start_date, " 20", "-"),
    end_date = str_replace(end_date, " 20", "-")
  ) %>%
  left_join(email_addr, by = c("project__id" = "project_number")) %>%
  mutate(email = ifelse(!is.na(contact), contact, email)) %>%
...
projects_re %>%
  ...
  distinct(project__id, .keep_all = TRUE) %>%
  write_ods(here("data-raw", "research-earth.ods"),
            sheet = "projects",
            append = TRUE)
...
df_institutes <- people %>%
  ...
  mutate(
    ...
    institute = glue::glue("{institute_name}, {institute_place}"),
...
gm_ids <- pull(gmaps, gm_id) %>% unique()
ntotal <- length(gm_ids)
websites <- tibble(
  gm_id = as.character(),
  gm_www = as.character()
)
...
lookup_org_proj_id <- p3_project_details %>%
...
lookup_org_proj_id %>%
  write_ods(
    here("data-raw", "research-earth.ods"),
    sheet = "organizations-projects-ref",
    append = TRUE)

# lookup for organizations and disciplines

lookup_organiz_discip_id <- p3_project_details %>%
  distinct(project_id, institute_id_p3) %>%
  filter(institute_id_p3 != 0) %>%
...
lookup_organiz_discip_id %>%
  write_ods(
    here("data-raw", "research-earth.ods"),
    sheet = "organizations-disciplines-ref",
    append = TRUE)
...
lookup_partners <- partners %>%
  ...
  summarize(partner_organizations_id = str_c(institute_id_p3.y,
    collapse = ","
  )) %>%
...
lookup_partners %>%
  write_ods(
    here("data-raw", "research-earth.ods"),
    sheet = "organizations-organizations-ref",
    append = TRUE)


organizations <- gmaps_clean %>%
  select(
  ...
  left_join(lookup_organiz_discip_id, by = "organizations_id") %>%
  left_join(lookup_partners, by = "organizations_id") %>%
  left_join(lookup_org_proj_id, by = "organizations_id")
...
organizations_clean <- organizations %>%
  ...
  distinct(organizations_id, .keep_all = TRUE)
...
specs <-
  ...
  map(read_csv)
names(specs) <- files
openxlsx::write.xlsx(specs, file = "data/research-earth.xlsx", overwrite = TRUE)
```
...
- [link 1](https://www.oecd.org/development/financing-sustainable-development/development-finance-standards/dacandcrscodelists.htm)
- [link 2](https://www.oecd.org/dac/financing-sustainable-development/development-finance-standards/DAC-CRS-CODES.xls)
