...
We downloaded the three datasets from [P3](http://p3.snf.ch/Pages/DataAndDocumentation.aspx) into [data-raw](./data-raw/).

```{r to-be-tested, eval=FALSE}
download_table_from_p3 <- function(csv_table, dest_folder = "data-raw") {
  url_table <- glue("http://p3.snf.ch/P3Export/{table}") %>%
...
grants <- read_p3_file(pluck(p3_files, 1))
people <- read_p3_file(pluck(p3_files, 2))
```
...
```{r, eval=FALSE}
r4d_grants <- grants %>% 
  filter(str_detect(funding_instrument, "r4d"))
...
r4d_ppl <-
  ...
  pivot_longer(
    ...
    values_drop_na = TRUE) %>%
  semi_join(r4d_grants, "project_number")
...
# 161 unique affiliations
write_rds(r4d, here("data", "r4d-in-p3.Rds"))
write_rds(affil, here("data", "partner-affil.Rds"))
```
